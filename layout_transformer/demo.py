...
class JSONLayout(Dataset):
    ...
    def render(self, layout, background):
        ...
        box[:, [0, 1]] = box[:, [0, 1]] / (self.size - 1) * 223
        ...
        for epoch in range(config.max_epochs):
            ...
            if self.config.samples_dir is not None and (epoch+1) % self.config.sample_every == 0:
                ...
                for i in self.fixed_x:
                    caption = []
                    for j in range(0,len(i[1:(i == 236).nonzero(as_tuple=True)[0][0]]),5):
                        caption.append(train_dataset.categories[i[1:(i == 236).nonzero(as_tuple=True)[0][0]][j].item()-224]['name'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--project_name', default="LayoutTransformer", help="wandb project name")
    ...
    # train_dataset = JSONLayout('/Data4/student_zhihan_data/Diffusion/datasets/GC10/train/_annotations.coco.json', max_length=50)
    train_dataset = JSONLayout('/Data4/student_zhihan_data/Diffusion/PCB/train/_annotations.coco.json', max_length=50)
    # valid_dataset = JSONLayout('/Data4/student_zhihan_data/Diffusion/datasets/GC10/valid/_annotations.coco.json', max_length=50)
    valid_dataset = JSONLayout('/Data4/student_zhihan_data/Diffusion/PCB/valid/_annotations.coco.json', max_length=50)
